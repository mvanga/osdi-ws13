...
A larger number of simpler cores additionally enables further power saving techniques,
at a more fine grained level than with large complex cores. It gives processors the
 ability to turn off cores (processor gating), or significantly lower the
voltage at which they operate (dynamic voltage scaling) to match processor utilisation. 
Trading off core complexity for  number of cores,
the designs make one core assumption: that the workloads they process
...
is bound by the sequential part of the application. If increasing the number of 
cores comes at the price of decreasing clock frequency, than the sequential
parts of the application will perform worse than before, penalising latency
...
Found that units hardwared to a paritcular data representatio 
or comptuational algorithm
can achieve 50 to 500 times grater enery efficency than a general 
purpose register organsiation. 
...
\paragraph{Specialisation vs Generality} One of emerging trends
...
Talk aout 10 x 10 rather than 90 x 10 
